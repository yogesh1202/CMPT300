...
int main(int argc, char *argv[]){
    ...
    local_hints.ai_protocol = IPPROTO_UDP; // Use UDP
    local_hints.ai_flags = AI_PASSIVE; // Obtain Host Server Info
    
    struct *peer, *my_machine;
    hints.ai_flags = 0; //find out why these two lines
    hints.ai_protocol = 0;
    int errno = getaddrinfo(argv[2], argv[3], &local_hints, &peer); // Client Server
    ...
    errno = getaddrinfo(NULL, argv[1], &local_hints, &my_machine); // Server Info
    ...
    errno = bind(socket_fd, my_machine->ai_addr, my_machine->ai_addrlen); // Get IP address of clients socket
    ...
    /* Pass arguments into a thread using
     ...
     2. File descriptor for client socket files
     */
    
    struct thread_constructor args(ListCreate(), peer, my_machine, socket_fd);

    pthread_t Tx, Rx, in_stream, out_stream; // Create 4 threads for each process
